...
static void GetCallStackDetails(const void* const* trace, int count, AllocList *newRecord) {
	for (int i = 0; i < count; ++i) {
		...
		printf(newRecord->details[i].symbol, 2048, "%s", "(No Symbol)");

		if (has_symbol) {
			printf(newRecord->details[i].symbol, 2048, "%s", symbol->Name);
		}
		if (has_line) {
			printf(newRecord->details[i].filename, 128, "%s", line.FileName);
			printf(newRecord->details[i].linenumber, 2048, " [%d]", line.LineNumber);
		}
		else {
			printf(newRecord->details[i].filename, 128, "%s", "Could not determing file name");
			printf(newRecord->details[i].linenumber, 2048, " [%d]", "Could not determine file line");
		}
	}
}
